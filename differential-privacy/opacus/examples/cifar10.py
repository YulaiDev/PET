    ...
    for i, (images, target) in enumerate(tqdm(train_loader)):
        ...
        if i % args.print_freq == 0:
            if not args.disable_dp:
                epsilon = privacy_engine.accountant.get_epsilon(delta=args.delta)# hier zien we hoeveel privacy verlies heeft de model
                print(
                    f"\tTrain Epoch: {epoch} \t"
...
def main():
    ...
    if not args.disable_dp:
        ...
        privacy_engine = PrivacyEngine(# dit zit dp aan 
        ...
        model, optimizer, train_loader = privacy_engine.make_private(
            ...
            max_grad_norm=max_grad_norm, # hoe hard je elke sample's gradient afkapt
            clipping=clipping,# clip per layer clip je alles tegelijk (flat) of laag per laag
            grad_sample_mode=args.grad_sample_mode, #beveriligde willekeur 
        )
